...
fn trace_ray(origin: vec3<f32>, direction: vec3<f32>, seed_in: u32) -> vec3<f32> {
  ...
  loop {
    ...
    let hit_pos = ro + rd * hit_t;
    if (uniforms.thermal_sensor.x >= 0.5) {
      if (hit_type == 1u && primitive_shape() == 6u) {
        let local_hit = quat_mul_vec(q_inv, hit_pos - cube_center);
        return thermal_image_plane_color(local_hit, uniforms.sphere_extent.xyz);
      }
      return vec3<f32>(0.0);
    }
    ...
    if (hit_type == 1u) {
      ...
      if (primitive_shape() == 4u) {
        albedo = image_plane_color(local_hit, uniforms.sphere_extent.xyz);
        L = L + throughput * albedo;
        break;
      }
      if (primitive_shape() == 6u) {
        albedo = thermal_image_plane_color(local_hit, uniforms.sphere_extent.xyz);
        L = L + throughput * albedo;
        break;
      }
    } else if (hit_type == 2u) {
    ...
    let primitive_is_mirror = hit_type == 1u &&
      (primitive_shape() == 2u || primitive_shape() == 5u ||
       (uniforms.sphere_params.z >= 0.5 && transmission < 0.05 && roughness <= 0.05));
...
fn trace_raytraced(origin: vec3<f32>, direction: vec3<f32>) -> vec3<f32> {
  ...
  if (!is_wine_scene) {
    ...
    for (var spi = 0u; spi < primitive_limit; spi = spi + 1u) {
      ...
      let prim_q_inv = vec4<f32>(-prim.rot.xyz, prim.rot.w);
      let local_shadow_origin = quat_mul_vec(prim_q_inv, shadow_origin - prim.pos.xyz);
      let local_to_light = quat_mul_vec(prim_q_inv, to_light);
      let t_local_sh = primitive_intersection_t_for(local_shadow_origin, local_to_light, prim.extent.xyz, prim.params, prim.lens);
      if (t_local_sh < cube_shadow_t) {
        cube_shadow_t = t_local_sh;
      }
    }
  }
  let visible = ((uniforms.mesh_enabled == 0u) || shadow_hit.kind == RAY_QUERY_INTERSECTION_NONE) && (cube_shadow_t >= 1e37);
  ...
  let primitive_is_mirror_simple = hit_type == 1u &&
    (primitive_shape_for(hit_primitive.params) == 2u ||
     primitive_shape_for(hit_primitive.params) == 5u ||
     (hit_primitive.params.z >= 0.5 && transmission < 0.05 && roughness <= 0.05));
  if (primitive_is_mirror_simple) {
    let face_n = normalize(select(normal, -normal, dot(rd, normal) > 0.0));
    let refl_dir = reflect(rd, face_n);
    let refl_col = select(sky(refl_dir) * 0.9, vec3<f32>(0.02, 0.01, 0.01), is_wine_scene);
    return refl_col * albedo + direct * 0.1;
  }

  if (transmission < 0.5) {
    let ambient = select(sky(normal) * 0.1 + vec3<f32>(0.02), vec3<f32>(0.01), is_wine_scene);
