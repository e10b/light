...
fn trace_raytraced(origin: vec3<f32>, direction: vec3<f32>) -> vec3<f32> {
  ...
  if (is_wine_scene) {
    ...
    let shadow_hit = rayQueryGetCommittedIntersection(&shadow_rq);
    let visible = ((uniforms.mesh_enabled == 0u) || shadow_hit.kind == RAY_QUERY_INTERSECTION_NONE);
    if (visible || hit_type == 3u) {
      let nl = max(dot(normalize(normal), to_light), 0.0);
      direct = direct + albedo * vec3<f32>(1.0, 0.82, 0.58) * spot_shape * 7.5 * nl * uniforms.sun_intensity;
    }
  } else {
    let count = max(uniforms.sun_light_count, 1u);
    for (var li = 0u; li < 8u; li = li + 1u) {
      ...
      if (visible) {
        let nl = max(dot(normalize(normal), to_light), 0.0);
        direct = direct + albedo * vec3<f32>(1.0, 0.94, 0.82) * nl * uniforms.sun_intensity;
      }
    }
  }
